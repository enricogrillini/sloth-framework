...
public class MonitorSingleton {
  ...
  private static volatile MonitorSingleton instance = null;
  ...
  private MonitorSingleton() {
    ...
    eventId = 1;
    runningEvent = new HashMap<Long, MonitorEvent>();
    statistics = new HashMap<String, MonitorStatistics>();
    trend = new LinkedHashMap<Timestamp, Integer>();
  ...
  public static MonitorSingleton getInstance() {
    if (instance == null) {
      synchronized (MonitorSingleton.class) {
        if (instance == null) {
  ...
  public synchronized long startEvent(String group, String name, User user) {
    if (active) {
      ...
      runningEvent.put(new Long(eventId), event);
  ...
  public synchronized void endEvent(long eventId)  {
    if (active && eventId > 0 & eventId < this.eventId) {
      MonitorEvent event = (MonitorEvent) runningEvent.remove(new Long(eventId));
      event.end();
      ...
      MonitorStatistics monitorStatistics = (MonitorStatistics) statistics.get(event.getGroup() + "." + event.getName());
      ...
      Timestamp ora = null;
      // FIXME - rimuovere il business exception
      try {
