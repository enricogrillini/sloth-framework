...
public class TagInput<T> extends InputField<List<T>> {

    public static final String DELIMITER = "|";

    public TagInput(String name, String description, DataTypes dataType) {
        super(name, description, dataType);
    }

    @Override
    public FieldType getFieldType() {
        return FieldType.TAG_INPUT;
    }
    ...
    public void copyToBffFields(BffFields bffFields) throws FrameworkException {
        if (bffFields != null) {
            bffFields.setString(getName(), getText());
        }
    }
...
//        }
    ...
    private void postString(String decodedText) throws FrameworkException {
        if (!isReadOnly()) {
            ...
            if (!BaseFunction.isBlank(getText())) {
                String[] texts = StringUtil.split(decodedText, DELIMITER);
                List<T> values = new ArrayList<>();
                for (String text : texts) {
                    T value = (T) getDataType().parseValue(text, getLocale(), getFormat());
                    if (value != null) {
                        values.add(value);
                    }
                }
    ...
    public String getText() throws FrameworkException {
        ...
        if (getValue() != null) {
            for (T value : getValue()) {
                ...
                stringBuilder.append(getDataType().formatValue(value, getLocale(), getFormat()));
            }
        }
